...
- ~~**README の `poll_interval`**~~〔**済**〕 — `README.md` の表が
  ...
  **修正済み**（本見直しで対応）。
- **smoke テストが「Edge/Chrome が無ければ緑」になる**〔**解消済み**（`--strict`）〕 —
  既定では Edge も Chrome も起動できないと `SKIP` かつ**終了コード 0** で抜ける（手軽な
  ローカル実行のため）。Edge→Chrome フォールバックは実装済み（コミット `b3176a9`）。
  これに加えて `--strict` オプション（`python tests/smoke_badge.py --strict`）を**追加済み**。
  付けるとブラウザ不在を **FAIL（終了コード 1）** として扱うので、CI に載せても
  「何も検証していないのに緑」にはならない。CI 本体（GitHub Actions で `--strict` を回す）は
  長期計画として未着手＝上の「CI がない」を参照。
