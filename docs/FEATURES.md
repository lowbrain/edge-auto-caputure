...
> **状態サマリ（2026-08-12 見直し）**: 機能提案は原則として未着手だが、
> **`F-C1`（ログイン状態の保持＝`profile_dir` オプトイン）は実装済み**。
> それ以外（`F-A*` `F-B*` `F-C2/C3` `F-D*`）は未対応のまま。
> 本文中の `file:line` 参照は初版当時のもので、現行（2026-08-12）へ更新した。

「ツールとして何ができるか」の観点での提案。
不具合・設計・運用の指摘は [`IMPROVEMENTS.md`](IMPROVEMENTS.md) 側にある。

**全体の優先順位は [`ROADMAP.md`](ROADMAP.md) が正。** 末尾の「優先度まとめ」は
本文書内での相対順位にすぎず、不具合対応との優先度は示していない
（機能追加は全体では第 5 群＝最後）。
...
`_shoot` が既に 3 経路の合流点（`edge_auto_capture.py:188`）なので、
引数を 1 個足すだけで通せる。

#### 実装時の注意（必ず踏む地雷）

- **CSV にするなら `encoding="utf-8-sig"` で書く。**
  BOM 無し UTF-8 で書くと **Excel で開いたときに日本語が文字化けする**。
  日本語環境では確実に踏む。
  （`IMPROVEMENTS.md` A-6 は「**読む**ときは `utf-8-sig`」の話。こちらは**書く**側。）
- **時刻は F-A4 のとおりオフセット付きで持つ。**
  ファイル名の `ts` をそのまま流用しないこと。
...
本ツールは headed 起動（`edge_auto_capture.py:117` の `headless=False`）。
...
### F-A4. タイムスタンプにタイムゾーンがない

ファイル名も `log.txt` も `datetime.now()` のローカル時刻で、
**オフセットの記録がない**（`capture.py:129-131`）。

証跡として第三者に渡す・他系統のログと時系列を突き合わせる用途では、
夏時間や別タイムゾーンの相手との照合で曖昧になる。

- **ファイル名**は現状維持でよい（`+09:00` は Windows のファイル名に使えない文字を含み、
  可読性も落ちる）
- **索引ファイル（F-A1）と log.txt** は ISO 8601 で持つ
  例: `2026-08-11T14:30:25.123+09:00`

F-A1 を実装するなら同時にやること。後から遡って直せない種類の情報である。
...
`capture.py:136` の `settle_delay` sleep の直後が差し込み位置。
...
### F-C1. ログイン状態が保持されない（設計判断が必要）〔**済**〕

> **状態: 実装済み**（コミット `3c1f5c0`）。`config.ini` に `profile_dir` を追加。
> 空なら従来どおり毎回まっさらな使い捨て（既定）、指定時のみそのフォルダを再利用して
> ログイン状態などを保持する（`config.py` の `profile_dir` / `edge_auto_capture.py:376-386`）。
> `cleanup_old_profiles` は `keep=` 引数で再利用プロファイルを掃除対象から除外する
> （`infra.py:166`）。以下は提案当時の記録。

毎回 `tempfile.mkdtemp` でまっさらな一時プロファイルを作っていた（当時 `edge_auto_capture.py:324`、
現行 `:385`）。これは「サインイン/同期/初回セットアップを回避」という**意図的な設計**だが、
裏返すと **認証が要るサイトは毎回ログインし直し**。業務調査で使うなら実質ブロッカーになり得た。

`config.ini` に `profile_dir`（指定時のみ再利用、空なら現状どおり使い捨て）を足すのが折衷案。
ただし `cleanup_old_profiles`（`infra.py:166`）が `edge-debug-*` を掃除する対象と
衝突しないよう分離が要る（`keep=` で対応済み。ただし使い捨て同士の同時起動衝突＝A-5 は別途残る）。

> **これは仕様の方針決定が先だった。** 「まっさら起動」を既定として維持しつつ、
> オプトインで永続化する形に決着した。
...
`badge.py:36` 付近の title（セレクタ入力欄のホバー説明）に**手順書**が埋まっていること自体が、
この導線の難しさを示している。
...
### F-D5. 表示文言が日本語ハードコード（今すぐの対応は不要）

`badge.py:21-71` に日本語が直接書かれている。

ただし設計としては `_BADGE_CONFIG`（`badge.py:60`）に集約済みで、
**差し替えやすい形にはなっている**。必要になった時点で辞書を言語別に持ち、
`build_badge_script` で選ぶだけで済む。

今すぐの対応は不要。配布先が日本語話者に限られない場合にのみ検討する。

> なお `USAGE.txt` は Shift-JIS、`config.ini` のコメントも日本語なので、
> 多言語化するなら**バーの文言だけでは足りない**点に注意。
...
| 提案 | 価値 | コスト | 備考 |
|------|------|--------|------|
| F-A1 索引ファイル | 大 | 小 | **最優先候補**。`utf-8-sig` と F-A4 を必ずセットで |
| F-A4 タイムゾーン付き時刻 | 中 | 極小 | F-A1 と同時に。後から遡って直せない |
...
| ~~F-C1 プロファイル永続化~~ | 大 | 中 | ✅**実装済み**（`profile_dir` オプトイン） |
| F-D1 セレクタピッカー | 大 | 中〜大 | 敷居を下げる |
| F-D5 多言語化 | 小 | 中 | 今すぐ不要。配布先次第 |

**最初の一手の推奨**: F-A1（索引）+ F-A4（時刻）+ F-B2（バナー除去）+ F-A2（`_part.png`）。
いずれも小さく入れられて保存物の実用度が明確に上がる。

~~F-C1 だけは方針決定が要るので、進める前に判断すること。~~ → **決着済み**
（既定は「まっさら起動」を維持し、`profile_dir` 指定時のみ永続化する形で実装）。
