...
宛先: 次に作業するモデル／担当者
作成日: 2026-08-11
起点コミット: `eac2b92`（ブランチ `refactor/spa-event-driven`、作業ツリーはクリーン）
...
4. **コードの修正はまだ 1 行も行っていない。** 追加したのは `docs/` の 3 ファイルのみ
   （いずれも未追跡・未コミット）。
...
未追跡ファイル: `docs/IMPROVEMENTS.md` / `docs/FEATURES.md` / `docs/HANDOFF.md`（本ファイル）。
これらを成果物に含めるかも利用者に確認すること。
