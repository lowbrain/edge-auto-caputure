...
> **かつての未決 3 件（`D-A1` ライセンス / `D-D1` 証明書 / `F-C1` プロファイル永続化）は
> 決定・対応済み**（2026-08）。`D-A1` は MIT 採用、`F-C1` は `profile_dir` オプトインで実装、
> `D-D1` は署名の受け口のみ用意（証明書取得は所有者判断で保留）。
> 対応の詳細は [`DISTRIBUTION.md`](DISTRIBUTION.md) を参照（旧 `DECISIONS.md` は削除済み）。
...
| **旧・未決 3 件（決定済み）** | `D-A1` ライセンス（MIT 採用）/ `F-C1` プロファイル永続化（実装済み）/ `D-D1` 証明書（署名受け口のみ用意・取得は所有者判断で保留）。詳細は [`DISTRIBUTION.md`](DISTRIBUTION.md) |
...
判断を待たずに進められた以下は**対応済み**（`build.ps1` に実装）。

- **`D-A2`** サードパーティ表記（`THIRD-PARTY-NOTICES.txt`）— 生成・同梱を実装済み
- **`D-D3`** ZIP の SHA256 併記 — 配布 ZIP と `*.zip.sha256` の生成を実装済み
