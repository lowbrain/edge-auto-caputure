...
Microsoft Edge で開いたページを、**記録ONの間だけ URL / タブが変わるたびに**、
フルページのスクリーンショット(`.png`)とページ全文テキスト(`.txt`)へ自動保存するツール。
キャプチャのタイミングは利用者が操作する。各ページ上部の操作パネルで
「記録開始／停止」（記録期間の制御）と「今すぐ1枚」（今のページを1回だけ撮る）を行える。
...
同じフォルダの `config.ini` で挙動を設定する。Edge を普通に閲覧し、ページ上部の
パネルで「記録開始」を押すと、以後 URL/タブの変化ごとに `output\` へ保存される
（「記録停止」で一時停止、「今すぐ1枚」で今のページを単発保存）。停止は Ctrl+C か Edge を閉じる。
